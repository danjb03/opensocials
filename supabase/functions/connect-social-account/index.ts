...
const PLATFORM_ACTORS = {
  // Keep official Apify actor as **primary** and a well-known community actor as **secondary backup**
  instagram: "apify/instagram-profile-scraper",
  tiktok: {
    // Official Apify actor (more actively maintained)
    primary: "apify/tiktok-scraper",
    // Community actor kept as fallback to ensure we can still collect metrics if primary fails
    secondary: "clockworks/tiktok-profile-scraper"
  },
  linkedin: {
    // Official Apify actor for personal / company profile scraping
    primary: "apify/linkedin-profile-scraper",
    // Community actor kept as fallback to harvest extra engagement metrics
    secondary: "ahmed-khaled/linkedin-engagement-scraper"
  },
  youtube: {
    // Official Apify YouTube channel scraper
    primary: "apify/youtube-channel-scraper",
    // Community actor as backup
...
serve(async (req) => {
  ...
  try {
    ...
    const usesDualActors = typeof PLATFORM_ACTORS[platform] === 'object';
    ...
    if (usesDualActors) {
      actorIds = {
        primary: PLATFORM_ACTORS[platform].primary,
        secondary: PLATFORM_ACTORS[platform].secondary
      };
      actorMetadata = JSON.stringify(actorIds);
    } else {
      actorIds = { primary: PLATFORM_ACTORS[platform] };
      actorMetadata = PLATFORM_ACTORS[platform];
    }
