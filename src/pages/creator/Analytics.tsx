...
const CreatorAnalytics = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('deal_earnings').select('amount, earned_at').eq('creator_id', user?.id).order('earned_at', {
        ascending: true
      });
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('deals').select('id, title, value, status, created_at, updated_at').eq('creator_id', user?.id).order('created_at', {
        ascending: false
      });
      if (error) throw error;
      return data;
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('brand_creator_connections').select('id, status, created_at, updated_at').eq('creator_id', user?.id).order('created_at', {
        ascending: false
      });
      if (error) throw error;
      return data;
  ...
  return <CreatorLayout>
      <div className="container mx-auto p-6 space-y-6">
        ...
        <EarningsOverview totalEarnings={totalEarnings} completedDeals={completedDeals} activeDeals={activeDeals} pipelineValue={pipelineValue} />
