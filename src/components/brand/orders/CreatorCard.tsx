...
const CreatorCard: React.FC<CreatorCardProps> = ({ 
  ...
  return (
    <div className="rounded-lg border border-border hover:border-border p-4 shadow-sm transition-all">
      <div className="flex justify-between items-start mb-4">
        <div className="flex items-center">
          <img 
            src={creator.imageUrl} 
            alt={creator.name} 
            className="h-12 w-12 rounded-full object-cover mr-3 border border-border shadow-sm"
          />
          <div>
            <p className="font-medium text-foreground">{creator.name}</p>
            <p className="text-sm text-foreground">{creator.platform}</p>
          </div>
        </div>
