...
import { CheckCircle, MessageSquare, Eye, AlertCircle, Loader2, RefreshCw } from 'lucide-react';
import { useCampaignSubmissions, useReviewSubmission } from '@/hooks/useSafeCampaignSubmissions';
...
const SafeCampaignReviewPanel: React.FC<CampaignReviewPanelProps> = ({
  campaignId,
  campaignName
}) => {
  ...
  // Safe hooks
  const { data: submissions, isLoading, isError, error, retry } = useCampaignSubmissions(campaignId);
  ...
  // Original CampaignReviewPanel functionality
    ...
    return (
      <Card>
        <CardContent className="pt-6 flex flex-col items-center justify-center text-center">
          ...
          <p className="text-gray-500 mb-4">The content review system is currently unavailable.</p>
    ...
    return (
      <Card>
        <CardContent className="pt-6 flex flex-col items-center justify-center text-center">
          ...
          <p className="text-gray-500 mb-4">{error?.message || 'Failed to load content submissions.'}</p>
          <Button variant="outline" size="sm" onClick={handleRetry}>
            <RefreshCw className="h-4 w-4 mr-2" />
            Try again
          </Button>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex space-x-2 pt-2">
                    <Button
                      ...
                      disabled={!supabaseReady || reviewSubmission.isPending}
                    >
                    ...
                    <Button
                      ...
                      disabled={!supabaseReady || reviewSubmission.isPending || (submission as any).revision_count >= 2}
                    >
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={!supabaseReady || reviewSubmission.isPending || (reviewAction === 'request_revision' && !feedbackText.trim())}
              className={
            ...
            >
              {reviewSubmission.isPending ? (
                ...
                reviewAction === 'approve' ? 'Approve' : 'Request Revisions'
              )}
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={!!previewFile} onOpenChange={() => setPreviewFile(null)}>
        <DialogContent className="sm:max-w-2xl">
          {previewFile?.type === 'image' ? (
            <img src={previewFile.url} className="w-full h-auto" alt="Content preview" />
          ) : (
